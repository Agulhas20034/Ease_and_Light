...
@Component({
  selector: 'app-cria-contas',
  templateUrl: './cria-contas.page.html',
  styleUrls: ['./cria-contas.page.scss'],
  standalone: false,
})
export class CriaContasPage implements OnInit {
  email: string = '';
  password: string = '';
  ...
  async createAccount() {
    ...
    try {
      ...
      if (this.telefone && !this.isTelefoneValid()) {
        this.showToast(this.t('phone_invalid'), 'warning');
        this.loading = false;
        return;
      }
      ...
      if (this.telefone) {
        ...
        if (telTaken) {
          this.showToast(this.t('phone_invalid') + ' — already in use', 'warning');
          this.loading = false;
          return;
        }
      }
      ...
      setTimeout(() => this.router.navigate(['/gere-contas']), 1500);
    } catch (error: any) {
